...
public class ScheduleDataManager : ILoadable
{
    ...
    private string AjustarAnioFecha(string dateStr, int offsetAnio = 0)
    {
        if (string.IsNullOrEmpty(dateStr)) return dateStr;

        try
        {
            DateTime fechaOriginal = DateTime.ParseExact(
                dateStr,
                "dd/MM/yyyy",
                System.Globalization.CultureInfo.InvariantCulture
            );

            int anioActual = DateTime.Now.Year + offsetAnio;

            // Se crea fecha dinámica
            DateTime fechaCorregida = new DateTime(
                anioActual,
                fechaOriginal.Month,
                fechaOriginal.Day,
                0, 0, 0
            ...
            return fechaCorregida.ToString("dd/MM/yyyy", System.Globalization.CultureInfo.InvariantCulture);
        }
        catch (Exception ex)
        {
            TFUtils.WarnLog("Error procesando fecha dinamica en " + dateStr + ": " + ex.Message);
            return dateStr;
        }
    }
    ...
    public IEnumerator Load()
    {
        ...
        foreach (Dictionary<string, object> scheduleDef in array)
        {
            ...
            string startDateCorregida = AjustarAnioFecha(startDate, 0);
            string endDateCorregida = AjustarAnioFecha(endDate, 0);

            try
            {
                if (DateTime.Parse(endDateCorregida) < DateTime.Parse(startDateCorregida))
                {
                    endDateCorregida = AjustarAnioFecha(endDate, 1);
                }
            }
            catch { }

            startDate = startDateCorregida;
            endDate = endDateCorregida;
